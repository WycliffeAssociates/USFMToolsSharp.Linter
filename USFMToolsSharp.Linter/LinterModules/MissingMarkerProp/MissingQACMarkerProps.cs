...
{
    public class MissingQACMarkerProps : ILinterModule
    {
        public List<LinterResult> Lint(USFMDocument input)
        {
            List<LinterResult> results = new List<LinterResult>();
            foreach(Marker marker in input.GetChildMarkers<QACMarker>())
            {
                if (String.IsNullOrEmpty(((QACMarker)marker).AcrosticLetter))
                {
                    results.Add(new LinterResult
                    {
                        Position = marker.Position,
                        Level = LinterLevel.Error,
                        Message = "Acrostic Letter is missing"
                    });
                }
            }
            return results;
