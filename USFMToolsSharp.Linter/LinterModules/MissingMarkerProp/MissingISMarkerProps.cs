...
{
    public class MissingISMarkerProps : ILinterModule
    {
        public List<LinterResult> Lint(USFMDocument input)
        {
            List<LinterResult> results = new List<LinterResult>();
            foreach(Marker marker in input.GetChildMarkers<ISMarker>())
            {
                if (String.IsNullOrEmpty(((ISMarker)marker).Heading))
                {
                    results.Add(new LinterResult
                    {
                        Position = marker.Position,
                        Level = LinterLevel.Error,
                        Message = "Introduction Section Heading is missing"
                    });
                }
            }
            return results;
